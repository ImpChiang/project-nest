...
Log4js.addLayout('Awesome-nest', function (logConfig) {
    return function (logEvent) {
        ...
        var positionOutput = position ? " [" + position + "]" : '';
        var typeOutput = "[" + logConfig.type + "] " + logEvent.pid.toString() + "   - ";
        var dateOutput = "" + Moment(logEvent.startTime).format('YYYY-MM-DD HH:mm:ss');
        var moduleOutput = moduleName ? "[" + moduleName + "] " : '[LoggerService] ';
        var levelOutput = "[" + logEvent.level + "] " + messageOutput;
        // 根据日志级别，用不同颜色区分
